...
if [ -z $1 ]
then
    echo "This script requires as input the number of copy files"
    exit
fi
...
echo "Entries greater than threshold: $greater_count"
smaller_count=$(echo $(($n-$greater_count)))
echo "Entries lower than threshold: $smaller_count"
